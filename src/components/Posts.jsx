...
const Post = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.error("Error in response from request:", response.statusText);
        }
      } catch (error) {
  ...
  // Fetch posts for each user when user data changes
  useEffect(() => {
      ...
      try {
        ...
        for (const user of userData) {
          ...
          if (response.ok) {
            ...
            console.error(
              "Error in response from request:",
              response.statusText
            );
          }
        }
  ...
  // Handle change in selected user
  const handleUserSelectChange = (event) => {
    setSelectedUserId(event.target.value);
  };

  // Filter posts based on selected user ID
